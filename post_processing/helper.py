...
def load_model_stgcnn(model_dir):
    model_registrar = ModelRegistrar(model_dir, 'cpu')
    save_path = os.path.join(model_dir, 'stgcnn_val_best_2.pth')
    print('')
    print('Loading from ' + save_path)
    model = stgcnn.Social_stgcnn()
...
def load_model_lstm(model_dir):
    model_registrar = ModelRegistrar(model_dir, 'cpu')
    save_path = os.path.join(model_dir, 'lstm_val_best.pth')
    print('')
    print('Loading from ' + save_path)
    model = lstm.LstmRNN()
    model.load_state_dict(torch.load(save_path))
