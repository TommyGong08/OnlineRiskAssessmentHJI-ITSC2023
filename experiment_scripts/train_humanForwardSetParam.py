...
dataset = dataio.ReachabilityHumanForwardParam(numpoints=65000, collisionR=opt.collisionR, velocity=opt.velocity,
                                               ...
                                               periodic_boundary=opt.periodic_boundary, diffModel=opt.diffModel)
...
def val_fn(model, ckpt_dir, epoch):
    ...
    for i in range(num_times):
        ...
        for j in range(num_controls):
            ...
            coords = torch.cat((time_coords, mgrid_coords, x_coords, y_coords, umin1_coords, umax1_coords, umin2_coords,
                                umax2_coords, umin3_coords, umax3_coords, umin4_coords, umax4_coords, umin5_coords,
                                umax5_coords), dim=1)
            ...
            s = ax.imshow(model_out.T, cmap='bwr', origin='lower', extent=(-1., 1., -1., 1.), vmin=-1., vmax=1.)
            fig.colorbar(s)
