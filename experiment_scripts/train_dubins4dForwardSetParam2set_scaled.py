...
import dataio, utils, training, loss_functions, modules, mfn, ODENet
...
p.add_argument('--module', type=str, default='SIREN', help='neural approximator',
               choices=['SIREN', 'MFN', 'GaborNet', 'ODENet'])
opt = p.parse_args()
...
if opt.module == 'SIREN':
    ...
    model = mfn.FourierNet(in_size=14, out_size=1, hidden_size=opt.num_nl, n_layers=opt.num_hl)
elif opt.module == 'GaborNet':
    model = mfn.GaborNet(in_size=14, out_size=1, hidden_size=opt.num_nl, n_layers=opt.num_hl)
elif opt.module == 'ODENet':
    model = ODENet.ODENet(data_dim=14, output_dim=1, hidden_dim=256, device=device)
model.cuda()
